...
async def submit_decision(task_id: str, request: DecisionRequest):
    ...
    try:
        ...
        await db_provider.update(
            table="man_tasks",
            record={"status": request.status.value, "decision": decision_payload},
            filters={"id": task_id},
        )

        # 2. Fetch Task (Supabase is real)
        task_res = (
            db_provider.client.table("man_tasks").select("*").eq("id", task_id).single().execute()
        )
        ...
        handle = client.get_workflow_handle(workflow_id=task["workflow_id"], run_id=task["run_id"])
