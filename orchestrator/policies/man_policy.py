...
class ManPolicy:
    ...
    CRITICAL_KEYWORDS = {
        "delete", "drop", "truncate", "transfer", "pay", "grant",
        "revoke", "shutdown", "terminate", "wipe", "format",
        "secret", "key", "token",
    }

    # High-impact tools that require supervision
    SENSITIVE_TOOLS = {
        "delete_record", "send_payment", "execute_sql", "deploy_contract"
    }
    ...
    def triage(cls, tool_name: str, tool_input: Dict[str, Any]) -> RiskTriageResult:
        ...
        found_keywords = [
            kw for kw in cls.CRITICAL_KEYWORDS if kw in input_str
        ]

        if found_keywords:
            reasons.append(
                f"Detected critical keywords: {', '.join(found_keywords)}"
            )
            risk_score += 0.5
