...
import { ChaosEngine, HEAVY_CHAOS_CONFIG, ChaosError, simulateFailure } from '../chaos-engine';
import { createEvent, EventEnvelope } from '../contracts';
...
class ManWorkflow {
    ...
    async execute(intent: string, params: Record<string, any>): Promise<{ status: string; reason?: string }> {
        const event = createEvent('orchestrator', 'action.intent')
